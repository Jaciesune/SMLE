    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            for i in range(len(images)):
                ...
                for anchor in range(predictions.shape[2]):
                    for grid_y in range(predictions.shape[0]):
                        for grid_x in range(predictions.shape[1]):
                            ...
                            # Sprawdzamy, czy pred_box ma wystarczająco dużo wartości
                            if predictions.shape[-1] < 5:
                                continue  # Pomijamy, jeśli tensor jest za krótki
                            
                            pred_box = predictions[grid_y, grid_x, anchor]
                            confidence = torch.sigmoid(torch.tensor(pred_box[4])).item()

                            # print(f"Confidence: {confidence:.4f}")  # Podgląd pewności predykcji

                            if confidence > 0.05:  # Tymczasowo zmniejszamy próg, by zobaczyć więcej
                ...
                plt.show()
                break  # Pokazujemy tylko pierwszą próbkę, żeby uniknąć nadmiarowych obrazków
...
def transform_annotations(boxes, image_width, image_height, transform):
    ...
    for box in boxes:
        ...
        # Transformacja bounding boxa
        #transformed_box = transform.apply_box([x1, y1, x2, y2])
        #Konwersja z powrotem do formatu (x_center, y_center, width, height)
        ...
        new_boxes.append([cls_id, x_center, y_center, width, height])
    return new_boxes

def train(epochs=20, batch_size=8, lr=1e-3, input_size=(416, 416)):
