...
def validate_model(model, dataloader, device, epoch, model_name):
    model.train()  # potrzebne do obliczania strat z targets
    total_val_loss = 0
    ...
    for idx, (images, targets) in enumerate(dataloader):
        ...
        # Przełączenie modelu w tryb ewaluacji i predykcja (bez strat)
        model.eval()
        with torch.no_grad():
        ...
        model.train()  # wróć do trybu treningowego dla następnej iteracji
